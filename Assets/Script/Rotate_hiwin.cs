...
using UnityEngine;
using System.IO;
using static Hiwin_loader;


public class Rotate_hiwin : MonoBehaviour
{
    // joint
    ...
    Hiwin_loader hiwin_data = new Hiwin_loader();

    public int count;

    void Start()
    {
        dataFilePath = Application.dataPath + "/DataSet/Demo/6to6_demo/predict_demo.json";
        count = 0;
        hiwin_data.txt_reader();
    }

    void Update()
    {       
        if (count < Hiwin_loader.coordinate_list.Length)
        {
            upperbaseJoint.localRotation = Quaternion.Euler(0, Hiwin_loader.coordinate_list[count, 0], 0);  // 0 a1 0
            torsoJoint.localRotation = Quaternion.Euler(Hiwin_loader.coordinate_list[count, 1], 0, 0);  // a2 0 0
            armJoint.localRotation = Quaternion.Euler(Hiwin_loader.coordinate_list[count, 2], 0, 0);  // a3 0 0
            forearmJoint.localRotation = Quaternion.Euler(0, Hiwin_loader.coordinate_list[count, 3], 0);  // 0 a4 0
            wristJoint.localRotation = Quaternion.Euler(Hiwin_loader.coordinate_list[count, 4], 0, 0);  // a5 0 0
            handJoint.localRotation = Quaternion.Euler(0, Hiwin_loader.coordinate_list[count, 5], 0);  // 0 a6 0
            Debug.Log(Hiwin_loader.coordinate_list[count, 0] + "," + Hiwin_loader.coordinate_list[count, 1] + "," + Hiwin_loader.coordinate_list[count, 2] + "," + Hiwin_loader.coordinate_list[count, 3] + "," + Hiwin_loader.coordinate_list[count, 4] + "," + Hiwin_loader.coordinate_list[count, 5]);
            count += 1;
        }

        else if (count == Hiwin_loader.coordinate_list.Length)
        {
            upperbaseJoint.localRotation = Quaternion.Euler(0, Hiwin_loader.coordinate_list[count, 0], 0);
            torsoJoint.localRotation = Quaternion.Euler(Hiwin_loader.coordinate_list[count, 1], 0, 0);
            armJoint.localRotation = Quaternion.Euler(Hiwin_loader.coordinate_list[count, 2], 0, 0);
            forearmJoint.localRotation = Quaternion.Euler(0, Hiwin_loader.coordinate_list[count, 3], 0);
            wristJoint.localRotation = Quaternion.Euler(Hiwin_loader.coordinate_list[count, 4], 0, 0);
            handJoint.localRotation = Quaternion.Euler(0, Hiwin_loader.coordinate_list[count, 5], 0);
